...
public class RegistrySounds {
    ...
    private static DeferredHolder<SoundEvent, SoundEvent> registerSound(String jsonName, SoundSource predefinedCategory) {
        ResourceLocation res = AstralSorcery.key(jsonName);
        return ASRegistries.SOUND_EVENTS.register(jsonName, () -> new CategorizedSoundEvent(res, predefinedCategory));
    }

    private static DeferredHolder<SoundEvent, SoundEvent> registerSound(String jsonName) {
        ResourceLocation res = AstralSorcery.key(jsonName);
        return ASRegistries.SOUND_EVENTS.register(jsonName, () -> SoundEvent.createVariableRangeEvent(res));
    }
}
